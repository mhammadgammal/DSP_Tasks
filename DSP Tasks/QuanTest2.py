def QuantizationTest2(file_name, Your_IntervalIndices, Your_EncodedValues, Your_QuantizedValues, Your_SampledError):
    ...
    if (len( Your_IntervalIndices ) != len( expectedIntervalIndices )
        ...
        return
    print('expectedIntervalIndices:', expectedIntervalIndices)
    for i in range( len( Your_IntervalIndices ) ):
        if (Your_IntervalIndices[i] != expectedIntervalIndices[i]):
            print('Your_IntervalIndices[i]:', Your_IntervalIndices[i])
            print('expectedIntervalIndices[i]:', expectedIntervalIndices[i])
            print( "QuantizationTest2 Test case failed, your signal have different indicies from the expected one" )
            return
    for i in range( len( Your_EncodedValues ) ):
        if (Your_EncodedValues[i] != expectedEncodedValues[i]):
            print('Your_EncodedValues[i]:', Your_EncodedValues[i])
            print('expectedEncodedValues[i]:', expectedEncodedValues[i])
            print(
